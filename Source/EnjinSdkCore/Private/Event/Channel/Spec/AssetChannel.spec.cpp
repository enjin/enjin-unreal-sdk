...
BEGIN_DEFINE_SPEC(FAssetChannelSpec,
                  "Enjin.Sdk.Event.AssetChannel",
...
END_DEFINE_SPEC(FAssetChannelSpec)

void FAssetChannelSpec::Define()
{
	Describe(TEXT("Channel"), [this]()
	{
		...
		{
			// Arrange
			const FString Expected = TEXT("enjincloud.test.asset.1");
			const FAssetChannel Channel(CreateDefaultPlatform(), TEXT("1"));
